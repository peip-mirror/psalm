        ...
        foreach ($array_type->types as &$type) {
            ...
            if ($type instanceof TArray || $type instanceof ObjectLike) {
                ...
                if ($assign_empty
                    ...
                    && ($string_key_value || $int_key_value !== null)
                ) {
                    // ok, type becomes an ObjectLike

                    $type = new ObjectLike([$string_key_value ?: $int_key_value => new Type\Union([new TEmpty])]);
                }

                if ($type instanceof TArray) {
                    // if we're assigning to an empty array with a key offset, refashion that array
                    if ($assign_empty && $type->type_params[0]->isEmpty()) {
                    ...
                    if ($assign_empty && $replacement_type) {
                    ...
                    if ($array_access_type->isEmpty() && !$assign_empty) {
                        if (IssueBuffer::accepts(
