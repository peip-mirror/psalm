    ...
    public static function updateArrayType(
        ...
        while ($child_stmts) {
            ...
            $child_stmt->inferredType = FetchChecker::getArrayAccessTypeGivenOffset(
                ...
                $child_stmt->dim ? $child_stmt->dim->inferredType : Type::getInt(),
                true,
                $array_var_id,
            ...
            foreach ($child_stmt->var->inferredType->types as $atomic_type) {
                if ($atomic_type instanceof TInt
                    ...
                    if (IssueBuffer::accepts(
                        new InvalidArrayAssignment(
                            'Cannot assign value on variable ' . $array_var_id . ' of scalar type ' . $type->getKey(),
                            new CodeLocation($statements_checker->getSource(), $stmt)
                        ),
                        $statements_checker->getSuppressedIssues()
                    )) {
        ...
        foreach ($reversed_child_stmts as $child_stmt) {
            if ($current_dim instanceof PhpParser\Node\Scalar\String_) {
                ...
                $array_assignment_type = new Type\Union([
                    new TArray([
                        $current_dim ? $current_dim->inferredType : Type::getInt(),
                        $current_type
                    ]),
                ]);
            }
        ...
        $root_array_expr->inferredType = Type::combineUnionTypes(
            $root_array_expr->inferredType,
            $array_assignment_type
        );

        if ($root_var_id) {
            $context->vars_in_scope[$root_var_id] = $root_array_expr->inferredType;
        }
